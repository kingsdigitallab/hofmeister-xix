<?xml version="1.0"?>
<!-- =========================================================== -->
<!--  MODULE:     proj_tei_com.xsl                                                                                     -->
<!--  VERSION:     1.0                                                                                                             -->
<!--  VERSION DATE:   $Id: proj_tei_com.xsl 970 2007-12-07 10:18:07Z elitta $       -->
<!--  VERSION CONTROL:                                                                                                   -->
<!-- =========================================================== -->
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  xmlns="http://www.w3.org/1999/xhtml" version="1.1" xmlns:filename="filename.uri"
  xmlns:ex="ex.uri" xmlns:vp="vp.uri" xmlns:tm="tm.uri" xmlns:pr="pr.uri"
  ...
  <xsl:template match="body/head"> </xsl:template>
  ...
  <xsl:template
    match="/TEI.2/teiHeader/fileDesc/titleStmt/*[self::author|self::editor|self::respStmt]"
    mode="pagehead">
    <xsl:apply-templates mode="pagehead" />
 ...
 -->
  ...
  <xsl:template match="div">
    <xsl:choose>
      ...
      <xsl:when test="@type = 'notice'">
        <blockquote class="callout callout_notice">
          <xsl:apply-templates />
        </blockquote>
      </xsl:when>
      <!--   Default  -->
  ...
  <xsl:template match="item[parent::list]">
    <xsl:choose>
      ...
      <xsl:when test="../@type='simple'">
        <!-- Note a simple list in TEI is often interpreted as a list of items on the same line. We
        don't support this in any special way any more. -->
        <xsl:apply-templates />
      </xsl:when>
      ...
      <xsl:when test="../@type='gloss'">
        <tr>
          <!-- item HERE -->
          <td>
            <xsl:call-template name="item-class-oddeven" />
          ...
          <td width="9" background="{$imgroot}{$genimgswitch}table_div.gif">
            <xsl:call-template name="item-class-oddeven" />
            <img alt="{$alt_text}" src="{$imgroot}{$genimgswitch}shim.gif" width="9"
              height="18" />
          </td>
          <!-- label HERE -->
          <td>
        ...
        <tr>
          <td background="{$imgroot}{$genimgswitch}h_div.gif" colspan="3" height="1"
            class="tablediv">
            <img alt="{$alt_text}" src="{$imgroot}{$genimgswitch}shim.gif" width="1"
              height="1" />
          </td>
        </tr>
      </xsl:when>
  ...
  <xsl:template match="label[parent::list]"> </xsl:template>
  ...
  <xsl:template match="headLabel">
    <tr>
      ...
      <td class="tablehead" width="9" background="{$imgroot}{$genimgswitch}table_div.gif">
    ...
    <tr>
      <td background="{$imgroot}{$genimgswitch}h_div.gif" colspan="3" height="1"
        class="tablediv">
        <img alt="{$alt_text}" src="{$imgroot}{$genimgswitch}shim.gif" width="1" height="1"
        />
      </td>
    </tr>
  </xsl:template>

  <xsl:template match="headItem"> </xsl:template>
      ...
      <xsl:when test="../../@type='thumbnail'">
        <dl>
          ...
          <dd>
            <p>
              <xsl:apply-templates
                select="//imagebase//image[@id=current()/figure/@url]/caption" />
            </p>
          </dd>
        </dl>
      </xsl:when>
    ...
    <xsl:choose>
      <xsl:when test="../../@type='special' or ../../@type='linkBib'">
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:text>r</xsl:text>
            <xsl:number level="single" count="row" format="01" />
          </xsl:otherwise>
        </xsl:choose>
      </xsl:when>
      <xsl:otherwise>
    ...
    <xsl:choose>
      ...
      <xsl:when test="../../@type='special' or ../../@type='linkBib'">
        <xsl:variable name="bcell">
          <xsl:number level="single" count="cell" />
        </xsl:variable>
        <xsl:variable name="ccell">
          <xsl:value-of select="$bcell - 1" />
        </xsl:variable>
        <xsl:text>c</xsl:text>
        <xsl:number value="$ccell" format="01" />
      </xsl:when>
      <xsl:otherwise>
        <xsl:text>c</xsl:text>
        <xsl:number level="single" count="cell" format="01" />
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="consistency-test">
    ...
    <xsl:for-each select="row[position()>1]">
      <xsl:variable name="cur-cell-count"
        select="count(cell) + sum(cell/@cols) - count(cell/@cols)" />
  ...
  <xsl:template name="table-complexDisplay">
    ...
    <xsl:choose>
      ...
      <xsl:when test="contains($error, '1')">
        <h3>Error converting table. Please check encoding for extra cells or missing
          colspans.</h3>
      </xsl:when>
      <!-- Output -->
      <xsl:otherwise>
        <div class="table">
          <div>
            <xsl:call-template name="table-class" />
            <table>
              ...
              <xsl:if test="row[@role='label']">
                <thead>
                  <xsl:apply-templates select="row[@role='label']">
                    <xsl:with-param name="number-of-rows"
  ...
  <xsl:template name="table-class">
    <xsl:attribute name="class">
      <xsl:choose>
        <xsl:when test="ancestor::wip | ancestor::album | ancestor::bibl">
          <xsl:text>t05</xsl:text>
        </xsl:when>
        <xsl:when test="@rend = 'highlight'">
          <xsl:text>t06</xsl:text>
        </xsl:when>
        <xsl:otherwise>
          <xsl:text>t01</xsl:text>
        </xsl:otherwise>
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <td>
          ...
          <xsl:apply-templates />
          <!-- To prevent empty elements in cocoon -->
          <xsl:text> </xsl:text>
        </td>
      </xsl:otherwise>
    </xsl:choose>
  ...
  <xsl:template name="cell-att">
    ...
    <xsl:variable name="col-position">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:text>c</xsl:text>
          <xsl:number value="$updated-position" format="01" />
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
  ...
  <!--
        ...
        The new position of the cell depends on the rowspans and colspans of the sibling cells and the
  cells in the
        previous rows.
    -->
    ...
    <xsl:choose>
      ...
      <xsl:when test="$count > 0">
        ...
        <xsl:variable name="new-count">
          <xsl:choose>
            <xsl:when
              test="ancestor::table/row[position() = $row]/cell[position() = $cell]">
        ...
        <xsl:variable name="new-pos">
          <xsl:choose>
            <xsl:when
              test="ancestor::table/row[position() = $row and $context-row > position()]/cell[position() = $cell and @rows]">
              ...
              <!-- 
                              The position of the context cell is updated if there are rowspans in previous cells of the previous
              rows and the
              ...
              <xsl:choose>
                <xsl:when
                  ...
                  <!-- 
                                        Creates the value that is added to the context cell position. Where there are colspans, it adds the
                  value of the colspan,
                                        otherwise it just increases the value of the context cell by one.
                                    -->
                  <xsl:variable name="step">
                    <xsl:choose>
                      <xsl:when
                        test="ancestor::table/row[position() = $row]/cell[position() = $cell and @cols]">
                        <xsl:value-of
                          select="ancestor::table/row[position() = $row]/cell[position() = $cell]/@cols"
                        />
                      </xsl:when>
        ...
        <!-- 
                    Recurs a call to the current template with the updated values of the context cell position,
        previous row position and previous cell
  ...
  <!-- IMG @ALT INFO -->
  <!-- Test for caption before description -->
  <xsl:template name="img-alt-cap-desc">
    <xsl:param name="img-caption" />
    <xsl:param name="img-desc" />
    <xsl:choose>
  ...
  <!-- Test for description before caption -->
  <xsl:template name="img-alt-desc-cap">
    <xsl:param name="img-caption" />
    <xsl:param name="img-desc" />
    <xsl:choose>
  ...
  <xsl:template name="showFigure">
    ...
    <xsl:variable name="img-thm-height"
      select="//imagebase//image[@id=current()/@url]/thmHeight" />
    <!-- Rendition info -->
    <!-- extra width to delimit captions for the inline thumbnails -->
    <xsl:variable name="img-thumb-width" select="$img-thm-width + 12" />
    ...
    <xsl:variable name="popup">
      <xsl:choose>
        <xsl:when
          test="contains(@rend, 'img') or ancestor::table[@type='thumbnail'][@rend='img']">
          <xsl:text>x87</xsl:text>
        </xsl:when>
        <xsl:when
          test="contains(@rend, 'html1') or ancestor::table[@type='thumbnail'][@rend='html1']">
          <xsl:text>x88</xsl:text>
        </xsl:when>
        <xsl:when
          test="contains(@rend, 'html2') or ancestor::table[@type='thumbnail'][@rend='html2']">
    ...
    <xsl:choose>
      ...
      <xsl:when test="ancestor::table/@type='thumbnail'">
        ...
        <a href="{$img-path-full}">
          ...
          <img src="{$img-path-thumb}">
            ...
            <!-- @alt info -->
            <xsl:call-template name="img-alt-cap-desc">
              <xsl:with-param name="img-caption" select="$img-caption" />
              <xsl:with-param name="img-desc" select="$img-desc" />
            </xsl:call-template>
            <!-- dimension info -->
            <!-- 
      ...
      <!-- Images with renditional information are treated differently, they can be thumbnails,
      thumbnails with captions or full sized images -->
      <xsl:when test="string(@rend)">
        <xsl:choose>
          <xsl:when test="@type='thumb'">
            <a href="{$img-path-full}">
              <xsl:attribute name="class">
                <xsl:value-of select="$left-right2" />
                <xsl:text> </xsl:text>
                <xsl:call-template name="img-popup">
                  <xsl:with-param name="popup" select="$popup" />
                </xsl:call-template>
              ...
              <span>&#160;</span>
              <img src="{$img-path-thumb}" class="s{$left-right}">
                <!-- @alt info -->
                <xsl:call-template name="img-alt-cap-desc">
                  <xsl:with-param name="img-caption" select="$img-caption" />
                  <xsl:with-param name="img-desc" select="$img-desc" />
          ...
          <xsl:when test="@type='thumb-caption'">
            <div class="figure">
              <div class="t{$left-right}">
                <dl style="width: {$img-thumb-width}px;">
                  <dt>
                    <!-- Full size popup -->
                    <a href="{$img-path-full}">
                      <xsl:attribute name="class">
                      ...
                      <img src="{$img-path-thumb}" class="s{$left-right}">
                        <!-- @alt info -->
                        <xsl:call-template name="img-alt-cap-desc">
                          <xsl:with-param name="img-caption"
                            select="$img-caption" />
                          <xsl:with-param name="img-desc"
                            select="$img-desc" />
                        </xsl:call-template>
                      </img>
                    </a>
                  </dt>
                  <xsl:if test="string($img-caption)">
                    <dd>
                      <xsl:value-of select="$img-caption" />
                    </dd>
                  </xsl:if>
          ...
          <xsl:otherwise>
            <img src="{$img-path-full}" class="s{$left-right}">
              <!-- @alt info -->
              <xsl:call-template name="img-alt-cap-desc">
                <xsl:with-param name="img-caption" select="$img-caption" />
                <xsl:with-param name="img-desc" select="$img-desc" />
      ...
      <xsl:when test="ancestor::table/@type='logo'">
        <img src="{$img-path-full}">
          <!-- @alt info -->
          <xsl:call-template name="img-alt-desc-cap">
            <xsl:with-param name="img-caption" select="$img-caption" />
            <xsl:with-param name="img-desc" select="$img-desc" />
          </xsl:call-template>
          <!-- dimension info -->
  ...
  <xsl:template match="xref">
    <!-- NOTE: the main types of links are now specified using @type. Any reference to @rend is
    legacy only, and kept in the code just in case, but @rend should not be used. -->

    <!-- VARIABLES FOR NORMAL INTERNAL LINKS -->
    <xsl:variable name="internal-link-path"
      select="//filebase//item[@id=current()/@from]/parent::*/groupHead/groupFolder" />
    <xsl:variable name="internal-link-filename"
      select="//filebase//item[@id=current()/@from]/fileName" />
    <xsl:variable name="internal-link-filetitle"
      select="//filebase//item[@id=current()/@from]/fileTitle" />
    ...
    <xsl:variable name="internal-link-group-path-2-prep" select="substring-before(@url, '-')" />

    <!-- For most documents, no extra directory needed -->
    <xsl:variable name="internal-link-group-path1"
      select="//filebase//group[@id=current()/@from]/groupHead/groupFolder" />

    <!-- For type03 documents, need extra directory to appear -->
    <xsl:variable name="internal-link-group-path2"
      select="concat($internal-link-group-path1, '/', $internal-link-group-path-2-prep)" />
    ...
    <xsl:choose>
      ...
      <xsl:when test="@type='external' or @rend='external'">
        <xsl:choose>
          ...
          <xsl:when test="@from">
            <xsl:variable name="cur-id" select="ancestor::TEI.2/@id" />
            <xsl:choose>
              <xsl:when test="@from[starts-with(.,$cur-id)]">
                <a href="#{@from}">
                  <xsl:call-template name="extWindow" />
                  <xsl:apply-templates />
                </a>
              ...
              <xsl:otherwise>
                <a>
                  <xsl:attribute name="href">
                    <xsl:value-of select="substring(@from,6,7)" />
      ...
      <xsl:when test="@type='email' or @rend='email'">
        <a href="mailto:{@url}" class="mail" title="Email link">
          <xsl:apply-templates />
        </a>
      </xsl:when>
      ...
      <xsl:when test="@type='internal' or @rend='internal'">
        <xsl:choose>
          <!--  Pointers to groups  -->
          <!-- This model could test for certain 'ids' only, instead of being so general as
          'starting with 'g'' -->
          <xsl:when test="starts-with(@from, 'g')">
            <a>
              <xsl:call-template name="intWindow">
                <xsl:with-param name="linked-filename"
                  select="$internal-link-group-filename" />
              </xsl:call-template>
              <xsl:attribute name="href">
          ...
          <xsl:otherwise>
            <a>
              <xsl:call-template name="intWindow">
                <xsl:with-param name="linked-filename"
                  select="$internal-link-filetitle" />
              </xsl:call-template>
              <xsl:attribute name="href">
                <xsl:value-of select="$linkroot" />
                <xsl:text>/</xsl:text>
      ...
      <!-- For some reason XMetal doesn't output default attributes, so treat all others as external
      links -->
      <xsl:otherwise>
        <a href="{@url}">
          <xsl:call-template name="extWindow" />
  ...
  <xsl:template match="ref">
    <!-- INTERNAL LINKS IN SAME OUTPUT DOCUMENT. This *won't* work if the pointer leads to a
    ...
    <sup>
      <a class="fnLink">
        <xsl:attribute name="href">
          ...
          <xsl:number level="any" format="01" from="text" />
        </xsl:attribute>
        <xsl:attribute name="id">
